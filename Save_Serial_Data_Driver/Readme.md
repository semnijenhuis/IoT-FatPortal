# Save serial data
In this character device driver the serial data is saved. There are open, close, read, and write funtions to use the driver as desired. The serial driver has mayor number of 251 to make it easier for setting the driver on startup. It also has a buffer size of 256, because that is enough for now to save the serial data in. If necessary this can be easily exstended in the top of serialDriver.c. The data buffer is cleared with every new write to not interact with the new message
...
To check if the compilation was succesfull you can run `readelf -h serialDriver.ko` and if the output shows ELF32 and MIPS R3000, as shown in the picture below, the program compiled succesfully. 
