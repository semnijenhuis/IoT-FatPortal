...
# Arguments
When you call the program you can enter parameters to specify the url of the FATportal and the productId to send to. This is done by `-p` or `--product` with the productId following for the productId, or `-u` or `--url` respectively with the url following. A example of how to do this: `./sendData -p acbf264b076c43c9aeb9a1d6d45a32a -u https://test.fatportal.nl/api/Ping`. If nothings is declared in the arguments the PRODUCTID and WEBSERVER are used for the productId and url.