...
static int __init my_init(void) {
    ...
    major = register_chrdev(MYMAJOR, "serialDriver", &fops);
    if (major < 0) {
        printk("serialDriver - Error registering chrdev\n");
        return major;
    }
    // Print message when succeeded
    printk("Started serialDriver on device number: %d\n", MYMAJOR);
    return 0;
}
...
static void __exit my_exit(void) {
    ...
    unregister_chrdev(MYMAJOR, "serialDriver");
}
...
MODULE_DESCRIPTION("A driver which can save up to 1024 bytes of data for the serial data");